...
class TransferSearchEngine:
    ...
    def search(
        ...
        searching_club: Optional[Club] = None
    ...
    def _sort_results(
        self,
        results: List[Tuple[Player, Dict]],
        criteria: SearchCriteria
    ) -> List[Tuple[Player, Dict]]:
        """Sort search results."""
        if criteria.sort_by == "value":
            ...
            results.sort(
                key=lambda x: x[0].attributes.get_overall(),
                reverse=criteria.sort_desc
            )
        elif criteria.sort_by == "potential":
            results.sort(
                key=lambda x: x[0].potential_ability,
                reverse=criteria.sort_desc
            )
    ...
    def _calculate_similarity_score(self, player1: Player, player2: Player) -> float:
        ...
        ability_diff = abs(
            player1.attributes.get_overall() - 
            player2.attributes.get_overall()
        )
        score -= ability_diff
