...
class PlayerValuationEngine:
    ...
    # Age value curves
    AGE_MULTIPLIERS = {
        16: 0.3, 17: 0.4, 18: 0.6, 19: 0.8, 20: 0.9,
        21: 1.0, 22: 1.1, 23: 1.2, 24: 1.3, 25: 1.4,
        26: 1.4, 27: 1.3, 28: 1.2, 29: 1.0, 30: 0.8,
        31: 0.6, 32: 0.4, 33: 0.3, 34: 0.2, 35: 0.15
    }
    ...
    def _calculate_ability_modifier(self, player: Player) -> float:
        ...
        overall = player.attributes.get_overall()
    ...
    def _calculate_potential_modifier(self, player: Player) -> float:
        ...
        current = player.attributes.get_overall()
    ...
    def _calculate_versatility_modifier(self, player: Player) -> float:
        ...
        strong_positions = sum(1 for rating in player.positions.values() if rating >= 15)
        playable_positions = sum(1 for rating in player.positions.values() if rating >= 10)
        
        if strong_positions >= 3:
            return 1.15
        elif strong_positions >= 2:
            return 1.08
        elif playable_positions >= 3:
            return 1.05
    ...
    def estimate_wage_demands(self, player: Player, transfer_fee: float) -> float:
        ...
        fee_factor = 1.0 + (transfer_fee / base_value - 1.0) * 0.3
        
        # Base wage calculation (roughly 0.5-2% of value per week)
        base_wage = base_value * 1000000 * 0.01 / 52  # 1% of value annually
        ...
        ability_factor = player.attributes.get_overall() / 70  # Normalized around 70
    ...
    def calculate_release_clause(self, player: Player, multiplier: float = 1.5) -> float:
        ...
        if player.age <= 23 and player.potential_ability - player.attributes.get_overall() > 10:
            multiplier *= 1.3
