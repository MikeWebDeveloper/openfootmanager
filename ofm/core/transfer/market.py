...
from ..db.models.transfer import (
    TransferListing, TransferNegotiation, TransferHistory,
    TransferWindow, TransferStatus, TransferType,
    ContractOffer, ContractStatus
)
from .valuation import PlayerValuationEngine
from .negotiation import TransferNegotiator, ContractNegotiator
from .search import TransferSearchEngine, SearchCriteria
...
class TransferMarket:
    ...
    def get_active_window(self) -> Optional[TransferWindow]:
        ...
        self._active_window = self.session.query(TransferWindow).filter(
    ...
    def list_player(
        ...
        existing = self.session.query(TransferListing).filter(
    ...
    def make_transfer_bid(
        ...
        existing = self.session.query(TransferNegotiation).filter(
        ...
        listing = self.session.query(TransferListing).filter(
            TransferListing.player_id == player.id,
            TransferListing.is_active == True
        ).first()
    ...
    def make_contract_offer(
        ...
        existing = self.session.query(ContractOffer).filter(
            ContractOffer.negotiation_id == negotiation.id
        ).first()
    ...
    def get_market_stats(self) -> Dict:
        ...
        stats['total_listings'] = self.session.query(TransferListing).filter(
            TransferListing.is_active == True
        ).count()
        
        # Count active negotiations
        stats['active_negotiations'] = self.session.query(TransferNegotiation).filter(
            TransferNegotiation.status == TransferStatus.NEGOTIATING
        ).count()
        ...
        if self.get_active_window():
            ...
            completed = self.session.query(TransferHistory).filter(
                TransferHistory.transfer_date >= window_start
            ).all()
    ...
    def simulate_deadline_day(self) -> List[Dict]:
        ...
        clubs = self.session.query(Club).filter(
            Club.transfer_budget > 1000000  # At least 1M budget
        ).all()
