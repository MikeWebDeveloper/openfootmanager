...
class AITransferManager:
        ...
        for position, min_required in position_requirements.items():
            ...
            if len(current_players) < min_required:
                ...
                needs.append(SquadNeed(
                    position=position,
                    priority=priority,
                    role=SquadRole.STARTER if len(current_players) == 0 else SquadRole.ROTATION
                ))
    ...
    def _attempt_signing(self, target: Dict) -> Optional[Dict]:
        ...
        # Make bid
        negotiation, msg = self.market.make_transfer_bid(
            player, self.club, bid_amount
        )
    ...
    def _assess_current_need(self, need: SquadNeed) -> int:
        ...
        current_players = [
            p for p in self.club.players
            if p.positions.get(need.position, 0) >= 15
        ]
    ...
    def _assess_player_importance(self, player: Player) -> SquadRole:
        ...
        position_players = [
            p for p in self.club.players
            if p.get_best_position() == position
        ]
        
        # Sort by ability
        position_players.sort(
            key=lambda p: p.attributes.get_overall(),
            reverse=True
        )
